...
def scaled_laplacian(A):
    n = A.shape[0]
    # check both options: 0+1 is degree=in and out a node, 0/1 is degree=in/out only.
    d = np.sum(A, axis=1)  # np.sum(A, axis=1)   np.sum(A, axis=0)+np.sum(A, axis=1)
    L = np.diag(d) - A
    for i in range(n):
        for j in range(n):
            if d[i] > 0 and d[j] > 0:
